...
namespace HyperEdit
{
    public class Lander : Window
    {
        ...
        private void LandAtTarget()
        {
            ...
            if (Si.TryParse(FindField<TextBox, string>("Lat"), out latitude) == false ||
                Si.TryParse(FindField<TextBox, string>("Lon"), out longitude) == false ||
                Si.TryParse(FindField<TextBox, string>("Alt"), out altitude) == false)
            {
